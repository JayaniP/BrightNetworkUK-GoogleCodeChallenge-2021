...
class VideoUnderProcess:
    def __init__(self):
        self.video = None
        self.status = VideoState.Stop
    ...
    def set_status(self, state):
        ...
        if self.status == VideoState.Playing:
            print("Playing video: " + self.video.title)
        elif self.status == VideoState.Pause:
            print("Pausing video: " + self.video.title)
        elif self.status == VideoState.Stop:
            print("Stopping video: " + self.video.title)
            self.video = None
        elif self.status == VideoState.Continue:
...
class VideoPlayer:
    ...
    def __init__(self):
        ...
        self.video_under_process = VideoUnderProcess() #currently playing video
    ...
    def play_video(self, video_id):
        ...
        if video != None:
            ...
            if video.flagged == None:
                if self.video_under_process.status != VideoState.Stop: #for avoiding the first time error print message from stop_video
                    self.stop_video() #stopping the current video if playing

                self.video_under_process.set_video(video, VideoState.Playing)
            else:
    ...
    def stop_video(self):
        ...
        if self.video_under_process.status != VideoState.Stop:
            self.video_under_process.set_status(VideoState.Stop)
    ...
    def pause_video(self):
        ...
        if self.video_under_process.video != None:
            if self.video_under_process.status != VideoState.Pause:
                self.video_under_process.set_status(VideoState.Pause)
            else:
    ...
    def continue_video(self):
        ...
        if self.video_under_process.video != None:
            if self.video_under_process.status == VideoState.Pause:
                self.video_under_process.set_status(VideoState.Continue)
            else:
        ...
        if self.is_playlist_exist(pln):
            ...
            if video != None:
                if video.flagged == None:
                    ...
                    print("Cannot add video to " + playlist_name + ": Video is currently flagged (reason: " + video.flagged + ")")
            else:
        ...
        if len(response_vid) != 0:
            ...
            if val.isnumeric():
                ...
                if _index > 0 and _index <= len(response_vid):
                    self.play_video(response_vid[_index-1].video_id)
            # else:
            #     print("")
        else:
            ...
            if video.flagged == None:
                ...
                if self.video_under_process.video != None:
                    if video_id == self.video_under_process.video._video_id:
                        if self.video_under_process.status == VideoState.Playing or self.video_under_process.status == VideoState.Pause:
                            self.video_under_process.set_status(VideoState.Stop)

                video.flagged = flag_reason
                print("Successfully flagged video:", video.title + " (reason: " + video.flagged + ")")
